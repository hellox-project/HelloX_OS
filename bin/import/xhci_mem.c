...
#define CACHELINE_SIZE DEFAULT_CACHE_LINE_SIZE
...
*xhci_alloc_container_ctx(struct xhci_ctrl *ctrl, int type)
{
	...
	ctx->size = (MAX_EP_CTX_NUM + 1) *
		CTX_SIZE(__readl(&ctrl->hccr->cr_hccparams));
	if (type == XHCI_CTX_TYPE_INPUT)
		ctx->size += CTX_SIZE(__readl(&ctrl->hccr->cr_hccparams));
...
struct xhci_container_ctx *ctx)
{
	...
	return (struct xhci_slot_ctx *)
		(ctx->bytes + CTX_SIZE(__readl(&ctrl->hccr->cr_hccparams)));
}
	...
	unsigned int ep_index)
{
	...
	return (struct xhci_ep_ctx *)
		(ctx->bytes +
		(ep_index * CTX_SIZE(__readl(&ctrl->hccr->cr_hccparams))));
}
