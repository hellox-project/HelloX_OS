...
int get_registers(struct r8152 *tp, u16 value, u16 index, u16 size, void *data)
{
	...
	int ret;
	char* tmp = (char*)_hx_aligned_malloc(size, ARCH_DMA_MINALIGN);
	if (NULL == tmp)
	{
		return -EINVAL;
	}
...
u16 ocp_read_word(struct r8152 *tp, u16 type, u16 index)
{
	...
	if (ret < 0)
	{
		_hx_printf("%s:generic_ocp_read failed,err = %d.\r\n",
			__func__,
			ret);
	}
...
int r8152_read_mac(struct r8152 *tp, unsigned char *macaddr)
{
	int ret;
...
int rtl8152_set_speed(struct r8152 *tp, u8 autoneg, u16 speed, u8 duplex)
{
	u16 bmcr, anar, gbcr;
...
void r8152b_get_version(struct r8152 *tp)
{
	u32 ocp_data;
	u16 tcr;
	int i;
...
int rtl_ops_init(struct r8152 *tp)
{
...
int r8152_send_common(struct ueth_data *ueth, void *packet, int length)
{
	...
	uint8_t* msg = (uint8_t*)_hx_aligned_malloc(length + sizeof(struct tx_desc),
		ARCH_DMA_MINALIGN);
	if (NULL == msg)
	{
		return -ENOMEM;
...
#ifndef CONFIG_DM_ETH
static int r8152_init(/*struct eth_device*/ __ETHERNET_INTERFACE *eth)
{
...
/*
* Initialization routine of R8152.
*/
...
int r8152_send(/*struct eth_device*/ __ETHERNET_INTERFACE *eth, void *packet, int length)
{
	struct ueth_data *dev = (struct ueth_data *)eth->pIntExtension;
...
/*
* Send a frame through the R8152 device,this is the
* internal version,it will be called by __r8152_send routine,which
* will be exposed to OS kernel.
*/
BOOL __r8152_internal_send(__ETHERNET_BUFFER* pEthBuff)
{
	...
	pEthInt = pEthBuff->pEthernetInterface;
	if (NULL == pEthInt)
	{
		BUG();
	}
	...
	ret = r8152_send(pEthInt, pEthBuff->Buffer, pEthBuff->act_length);
	if (0 == ret)
	{
		bResult = TRUE;
	}
	...
	{
		pEthInt->ifState.dwTxErrorNum += 1;
		/*
		* Show a warning message if the first time.
		*/
		if (1 == pEthInt->ifState.dwTxErrorNum)
		{
			_hx_printf("%s:send frame failed[err = %d,if_n = %s].\r\n",
				__func__,
				ret,
				pEthInt->ethName);
		}
...
__TERMINAL:
	/*
	* Should destroy the ethernet buffer,since it is processed over.
	*/
...
/*
* Send frame routine which is expose to OS kernel.
*/
BOOL __r8152_send(__ETHERNET_INTERFACE* pEthInt)
{
	...
	if (pEthInt->SendBuffer.pEthernetInterface != pEthInt)
	{
		BUG();
	}
	...
	*/
	...
	if (NULL == ss->pSndList)  //First element.
	{
		...
		* of this ethernet interface.
		*/
		msg.wCommand = R8152_SEND_FRAME;
		msg.dwParam = (DWORD)pNewBuff;
		bResult = SendMessage(
			(HANDLE)ss->pTxThread,
			&msg);
		if (!bResult)  /* Target kernel thread's msg queue full. */
		{
			/*
			* Remove it from list.
			*/
...
__ETHERNET_BUFFER* __r8152_recv(__ETHERNET_INTERFACE *eth)
{
	struct ueth_data *dev = (struct ueth_data *)eth->pIntExtension;
	...
	/*
	* Allocate receiving buffer,it should be aligned to 4K to make sure
	* the whole qTD's xfer space can be used,since each frame in qTD must
	* align to 4K.
	*/
	recv_buf = (uint8_t*)_hx_aligned_malloc(RTL8152_AGG_BUF_SZ,
		USB_DMA_MINALIGN);
	if (NULL == recv_buf)
	{
		goto __TERMINAL;
...
static void r8152_halt(/*struct eth_device*/ __ETHERNET_INTERFACE *eth)
{
	struct ueth_data *dev = (struct ueth_data *)eth->pIntExtension;
	struct r8152 *tp = (struct r8152 *)dev->dev_priv;
...
static int r8152_write_hwaddr(/*struct eth_device*/ __ETHERNET_INTERFACE *eth)
{
	struct ueth_data *dev = (struct ueth_data *)eth->pIntExtension;
	struct r8152 *tp = (struct r8152 *)dev->dev_priv;
	...
	memcpy(enetaddr, eth->ethMac, ETH_ALEN);
...
BOOL __r8152_control(__ETHERNET_INTERFACE* pEthInt, DWORD dwOperations, LPVOID pData)
{
	...
	if ((NULL == pEthInt) || (0 == dwOperations))
	{
		goto __TERMINAL;
	}
	switch (dwOperations)
	{
	case R8152_CTRL_HALT:
		//Just call r8152_halt.
		bResult = TRUE;
		break;
	case R8152_CTRL_WRITE_HWADDR:
		//Just call r8152_write_hwaddr routine.
		bResult = TRUE;
		break;
	default:
		break;
