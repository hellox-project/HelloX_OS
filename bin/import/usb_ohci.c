...
#endif /* DEBUG */
#define err(format, ...) printf("ERROR: " format "\n", __VA_ARGS__)
#ifdef SHOW_INFO
...
static int ep_link(ohci_t *ohci, ed_t *edi)
{
	...
	case PIPE_CONTROL:
		...
		if (ohci->ed_controltail == NULL)
			ohci_writel((u32)ed, &ohci->regs->ed_controlhead);
		else
	...
	case PIPE_BULK:
		...
		if (ohci->ed_bulktail == NULL)
			ohci_writel((u32)ed, &ohci->regs->ed_bulkhead);
		else
...
struct usb_device *dev, int index, urb_priv_t *urb_priv)
{
	...
	if (data)
		td->hwBE = m32_swap((unsigned long)((char*)data + len - 1));
	else
		td->hwBE = 0;
	...
	* bits for reseting */
	...
	case PIPE_BULK:
		...
		while (data_len > 4096) {
			...
			(char*)data += 4096; data_len -= 4096; cnt++;
		}
		info = usb_pipeout(pipe) ?
...
#ifdef __MS_VC__
			len = min(leni,
			(int)min(sizeof(root_hub_dev_des),
			wLength));
#else
...
#ifdef __MS_VC__
			len = min(leni,
			(int)min(sizeof(root_hub_config_des),
			wLength));
#else
			len = min_t(unsigned int,
			leni,
...
#ifdef __MS_VC__
				len = min(leni,
					(int)min(sizeof(root_hub_str_index0),
					wLength));
#else
				len = min_t(unsigned int,
					leni,
...
#ifdef __MS_VC__
						  len = min(leni,
							  (int)min(sizeof(root_hub_str_index1),
							  wLength));
#else
						  len = min_t(unsigned int,
							  leni,
...
static __COMMON_USB_CONTROLLER* CreateUsbCtrl(LPVOID pCtrl)
{
	...
	return USBManager.CreateUsbCtrl(&ctrlOps, USB_CONTROLLER_OHCI,NULL,pCtrl);
}
...
int _ohci_usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
	...
	//Allocate ghcca from system heap,it's alignment must be 256 according to USB specification,
	//I don't know the reason why follow this rule.
	ghcca = (struct ohci_hcca*)memalign(256, sizeof(struct ohci_hcca));
	if (NULL == ghcca)
	{
		_hx_printf("Can not allocate HCCA global regin.\r\n");
		goto __TERMINAL;
	}
	...
	id.Bus_ID.PCI_Identifier.dwClass = 0x0C0310 << 8;
	
	//Get the USB controller device in system.
	pUsbDev = DeviceManager.GetDevice(&DeviceManager,
		BUS_TYPE_PCI,
		&id,
		NULL);
	...
	{
		_hx_printf("Can not find any OHCI controller in system.\r\n");
		goto __TERMINAL;
	}
#else
	...
	*controller = CreateUsbCtrl(&gohci);
	...
	ohci_inited = 1;
__TERMINAL:
	if (!ohci_inited)  //Failed to initialize the OHCI controller.
	{
		if (ghcca)  //Release it.
		{
			_hx_free(ghcca);
