...
static HANDLE hDhcpThread = NULL;
	...
	struct sockaddr_in client_addr)
{
	...
	else if (message_type == DHCP_REQUEST)
	{
		...
		*dhcp_opt++ = 0x80;
		...
		int dn_len = strlen(HX_DEFAULT_DOMAIN_NAME);
		*dhcp_opt++ = DHCP_OPTION_DOMAIN_NAME;
		*dhcp_opt++ = dn_len;
		strcpy(dhcp_opt++, HX_DEFAULT_DOMAIN_NAME);
		dhcp_opt += dn_len;
...
{
	__DHCP_INTERFACE* pInt = (__DHCP_INTERFACE*)parameter;
	int sock, max_sock;
	int bytes_read;
	char *recv_data;
	uint32_t addr_len;
	...
	while (TRUE)
	{
		...
		while (TRUE)
		{
			...
			if (sel_ret <= 0)
			{
				//_hx_printf("select time out or err[%d].\r\n", sel_ret);
		...
		//_hx_printf("select break out with ret[%d]\r\n", sel_ret);
...
static unsigned long __GetGenifByName(char* genif_name)
{
	...
	while (TRUE)
	{
		ret_val = GetGenifInfo(pGenifSnap, &buff_req, 0);
		if (-1 == ret_val)
		{
...
static BOOL StartDhcpIf(__DHCP_INTERFACE* pIf)
{
	...
	if (0 != AddGenifAddress(pIf->genif_index,
	...
	_hx_printf("%s:set addr OK\r\n", __func__);

	/* New a server sock and listen on it. */
		/* create a socket with UDP */
	if ((sock = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
	{
		DEBUG_PRINTF("Sock fail on if[%d].\r\n",
			pIf->if_index);
		goto __TERMINAL;
	}
	_hx_printf("%s:create sock[%d] OK\r\n", __func__, sock);
	...
	_hx_printf("%s:set sock option OK\r\n", __func__);
	...
	{
		/* bind failed. */
		DEBUG_PRINTF("Bind server address fail on if[%d].\r\n",
			pIf->if_index);
		goto __TERMINAL;
	}
...
static BOOL dhcp_server_start(char* netif_name)
{
	...
	if (NULL == hDhcpThread)
	{
		...
		if (NULL == hDhcpThread)
		{
			_hx_printf("Start dhcp service failed.\r\n");
			goto __TERMINAL;
		}
	}
	_hx_printf("%s:create dhcp thread OK\r\n", __func__);
