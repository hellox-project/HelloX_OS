...
LPBYTE LoadAppToMemory_PE(HANDLE hFileObj)
{
	...
	ReadFile(hFileObj,sizeof(ImageDosHeader),&ImageDosHeader,&dwReadSize);	
	...
	ReadFile(hFileObj,dwSecTableSize,pSectionHdr,&dwReadSize);
...
BOOL AppFormat_PE(HANDLE hFileObj)
{
	...
	ReadFile(hFileObj,sizeof(szPeFlage),szPeFlage,&dwRead);

	if(szPeFlage[0] == 'M' && szPeFlage[1] == 'Z' )
	{
		return TRUE;
	}
