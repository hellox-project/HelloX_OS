...
#ifdef __MS_VC__
__declspec(align(ARCH_DMA_MINALIGN)) static ccb usb_ccb;
#else
...
//Make this array as global since it will be refered in usbdev_storage.c file.
//static block_dev_desc_t usb_dev_desc[USB_MAX_STOR_DEV];
block_dev_desc_t usb_dev_desc[USB_MAX_STOR_DEV] = { 0 };
	...
	while (length) {
		...
		do {
			...
			debug("Bulk xfer 0x%lx(%d) try #%d\r\n",
				//For debugging,comments the following line,and adds a new line.
	...
	lbaint_t blkcnt, const void *buffer)
{
	...
	//debug("usb_write: dev %d startblk " LBAF ", blccnt " LBAF
	//	" buffer %" PRIxPTR "\r\n", device, start, blks, buf_addr);
