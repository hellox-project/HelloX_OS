...
void MD5Update(MD5Context *s, unsigned char const *p, unsigned len)
{
...
void MD5Final(unsigned char output[16], MD5Context *s)
{
    int i;
    unsigned pad;
    unsigned char c[64];
...
{
    return snewn(3, MD5Context);
}

void hmacmd5_free_context(void *handle)
{
    smemclr(handle, 3*sizeof( MD5Context));
    sfree(handle);
}

void hmacmd5_key(void *handle, void const *keyv, int len)
{
    MD5Context *keys = ( MD5Context *)handle;
...
void hmacmd5_do_hmac(void *handle, unsigned char const *blk, int len,
		     unsigned char *hmac)
{
    hmacmd5_do_hmac_internal(handle, blk, len, 0, 0, hmac);
}
