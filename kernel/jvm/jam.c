...
//HelloX Porting: HelloX common header file.
#include <stdafx.h>
#include <kapi.h>
...
#include <string.h>
#include <stdlib.h>
//HelloX Porting: No need this file.
//#include <stdarg.h>
#include <io.h>
...
int parseCommandLine(int argc, char *argv[], InitArgs *args) {
	...
	//Allocate props array under HelloX.
	//HelloX porting code.
	props = (Property*)sysMalloc(sizeof(Property) * (argc - 1));
	if(0 == props)
	{
		goto exit;
	}
...
exit:
    ...
    exit(status);
	return 0;  //To eliminate the compiling warning(not all control paths return a value).
}

//HelloX Porting: Change the main name since it may conflict with OS.
