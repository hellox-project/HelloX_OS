...
//    Module Name               : fdisk2.c
//    Module Funciton           : 
//    Description               : This file is the second part of FDISK application 
//                                source code, format command is implemented in this file.
...
/*
 * Typical instance of __BPB_FAT32BS object.
 * In most case of format,only modify several variables
 * of this object and write it into the first sector 
 * of target partition is ok.
 */
...
/*
 * The following code is captured from MS's FAT32 SPEC,
 * to determine the default sector per cluster number
...
/*
 * Searchs the DskTableFAT32 array by using dwPartSectNum,
 * returns the appropriate SecPerClus counter.
 */
...
/*
 * Calculate sector number occupied by one FAT.
 * This is a local helper routine,only suitable for FAT32.
 * This algorithm is copied from MS's FAT32 SPEC,
 * and only keep the FAT32 related part.
 */
...
/* Show out progressing information. */
...
/*
 * Helper routine to initialize the FAT zone of target partition,
 * and root directory also be created.
 */
	...
	BYTE             VolLabel[])    //Volume label.
{
	...
	/*
	 * Write the FAT table into partition,
	 * the first three entry(cluster 0,1,2) are
	 * initialized to EOC.
	 */
	...
	*pFatEntry = 0xFFFFFFFF;
	pFatEntry++;
	*pFatEntry = 0xFFFFFFFF;
	/* Write the master FAT table. */
	if (!__WriteDeviceSector((__COMMON_OBJECT*)pPartition,
		dwFatStart,
		1,
		Buffer))
	{
	...
	/* Initialize the rest part of FAT table,all to zero. */
	memzero(&Buffer[0], 512);
	...
	/*
	 * Initialize the root directory,assume
	 * the start cluster number of root is 2.
	 */
	...
	/* write the first sector of root directory's first cluster. */
	if (!__WriteDeviceSector((__COMMON_OBJECT*)pPartition,
		dwRootStart,
		1,
		Buffer))
...
static VOID formatusage()
{
	...
	PrintLine("    Where:");
	PrintLine("      partition_name : Partition device name.");
	PrintLine("      fs_type        : FS type.");
	PrintLine("      vol_label      : Volume label,default is 'NO NAME    '");
}

/* 
 * Format executing routine for FAT32 file system.
 * This routine takes the following actions:
 *  1. Get the Sector per cluster value by looking up DksTableFAT32 array,
 *     using partition's sector number;
 ...
 *  4. Calculate the sector counter occupied by one FAT table and fill it 
 *     into BPB_FATSz32 of Fat32Sector0;
 *  5. Set the BS_VolID of Fat32Sector0 to AAAA,the simple way:-);
...
DWORD format(__CMD_PARA_OBJ* pCmdParam)
{
	...
	if (pCmdParam->byParameterNum == 4)
	{
		/* Volumn label is specified. */
		nVolLen = StrLen(pCmdParam->Parameter[3]);
		if (nVolLen > 11)
		{
			/* 11 ASCII characters at most. */
			for (i = 0; i < 11; i++)
			{
	...
	if (NULL == pPartition)
	{
		_hx_printf("  No partition found.\r\n");
		return FDISK_CMD_SUCCESS;
	}
	/* Verify the partition oblect. */
	if (0 == (pPartition->dwAttribute & DEVICE_TYPE_PARTITION))
	{
		_hx_printf("  Not a partition.\r\n");
	...
	GotoHome();
	ChangeLine();
	if (!Fat32Format(pPartition, VolLabel, 0))
	{
		_hx_printf("  Format fail.\r\n");
	}
	else
	{
		_hx_printf("  Format OK.\r\n");
	}
	IOManager.CloseFile((__COMMON_OBJECT*)&IOManager,
		(__COMMON_OBJECT*)pPartition);
	return FDISK_CMD_SUCCESS;
