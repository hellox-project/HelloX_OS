...
/*
 * Ported to HelloX by Garry.Xin, all licenses and
 * copyrights are keep unchanged.:-)
 */
...
#define upper_32_bits(n) (__u32)((n) >> 32)
#endif
...
#ifndef udelay
#define udelay __MicroDelay
#endif

#ifndef mdelay
static void mdelay(sec)
{
	__MicroDelay(sec * 1000);
}
#endif

#define virt_to_bus(devno, v)	VIRTUAL_MEMORY_TO_PCI(devno,v) //pci_virt_to_mem(devno, (void *) (v))
#define bus_to_phys(devno, a)	PCI_TO_VIRTUAL_MEMORY(devno,v) //pci_mem_to_phys(devno, a)
...
*/
#ifdef __MS_VC__
//__declspec(align(E1000_BUFFER_ALIGN)) struct e1000_tx_desc tx_base[16] = { 0 };
//__declspec(align(E1000_BUFFER_ALIGN)) struct e1000_rx_desc rx_base[16] = { 0 };
//__declspec(align(E1000_BUFFER_ALIGN)) unsigned char packet[4096] = { 0 };
#else
ALLOC_ALIGN_BUFFER(struct e1000_tx_desc, tx_base, 16, E1000_BUFFER_ALIGN);
ALLOC_ALIGN_BUFFER(struct e1000_rx_desc, rx_base, 16, E1000_BUFFER_ALIGN);
ALLOC_ALIGN_BUFFER(unsigned char, packet, 4096, E1000_BUFFER_ALIGN);
#endif

//static int tx_tail = 0;
//static int rx_tail = 0, rx_last = 0;
...
void e1000_standby_eeprom(struct e1000_hw *hw)
{
	...
	if (eeprom->type == e1000_eeprom_microwire) {
		...
		E1000_WRITE_REG(hw, EECD, eecd);
		E1000_WRITE_FLUSH(hw);
		__MicroDelay(eeprom->delay_usec);
...
e1000_detect_gig_phy(struct e1000_hw *hw)
{
	...
	* the routines below will figure this out as well. */
	...
	DEBUGOUT("PHY ID:0x%X,MAC type:0x%X\r\n", hw->phy_id,hw->mac_type);
...
e1000_sw_init(struct e1000_hw *hw)
{
	...
	if (result) {
		E1000_ERR(hw, "Unknown MAC Type[dev = 0x%X,vdr = 0x%X].\r\n",
			hw->device_id,
			hw->vendor_id);
		return result;
	}
...
e1000_configure_tx(struct e1000_hw *hw)
{
	...
	E1000_WRITE_REG(hw, TDBAL, lower_32_bits((unsigned long)hw->tx_base));
	E1000_WRITE_REG(hw, TDBAH, upper_32_bits((unsigned long)hw->tx_base));
...
#define DEFAULT_ITR		1000000000/(MAX_INTS_PER_SEC * 256)
	...
	E1000_WRITE_REG(hw, RDBAL, lower_32_bits((unsigned long)hw->rx_base));
	E1000_WRITE_REG(hw, RDBAH, upper_32_bits((unsigned long)hw->rx_base));
...
int _e1000_transmit(struct e1000_hw *hw, void *txpacket, int length)
{
...
int _e1000_init(struct e1000_hw *hw, unsigned char enetaddr[6])
{
	int ret_val = 0;
	...
	if (ret_val < 0) {
		if ((ret_val == -E1000_ERR_NOLINK) ||
			...
			E1000_ERR(hw, "Hardware init failed with err[%d]\r\n",
				ret_val);
		}
		return ret_val;
	}
...
int e1000_init_one(struct e1000_hw *hw, int cardnum, __PHYSICAL_DEVICE* pdev,
	unsigned char enetaddr[6])
{
	...
	pci_read_config_dword(pdev, PCI_CONFIG_OFFSET_BASE1, &val);
	E1000_DBG(hw, "iobase 0x%08x\r\n", val & 0xfffffff0);

	/* Try to enable I/O accesses and bus-mastering */
	val = PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER;
	//pci_write_config_dword(pdev, PCI_COMMAND, val);
	pci_write_config_word(pdev, PCI_COMMAND, (unsigned short)val);
	...
	hw->hw_addr = pci_map_bar(pdev, PCI_CONFIG_OFFSET_BASE1, 0);
		//PCI_REGION_MEM);
...
{
	_hx_sprintf(str, "E1000_Eth_If_%d", cardnum);
}
...
***************************************************************************/
static int e1000_transmit(__ETHERNET_INTERFACE* nic, void *txpacket, int length)
{
	struct e1000_hw *hw = nic->pIntExtension;
...
***************************************************************************/
static void
e1000_disable(__ETHERNET_INTERFACE *nic)
{
	struct e1000_hw *hw = nic->pIntExtension;
	_e1000_disable(hw);
}
...
***************************************************************************/
static int
e1000_init(__ETHERNET_INTERFACE* nic, void *bis)
{
	struct e1000_hw *hw = nic->pIntExtension;

	return _e1000_init(hw, &nic->ethMac[0]);
}

int e1000_poll(__ETHERNET_INTERFACE* nic)
{
	struct e1000_hw *hw = nic->pIntExtension;
	int len;
	...
	len = _e1000_poll(hw);
	if (len) {
		net_process_received_packet(nic, (unsigned char *)hw->packet, len);
		fill_rx(hw);
	}
...
e1000_initialize(void * bis)
{
	...
	for (i = 0; (devno = pci_find_devices(e1000_supported, i)) >= 0; i++) {
		...
		*/
		...
		if (!nic || !hw) {
			_hx_printf("e1000#%u: Out of Memory!\r\n", i);
			free(nic);
			free(hw);
			continue;
		}
