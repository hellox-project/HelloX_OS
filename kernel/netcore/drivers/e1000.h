...
/*
 * Ported to HelloX by Garry.Xin, all license and copyrights are
 * keep unchanged.:-)
 */
...
#define E1000_ERR(NIC, fmt, ...) \
	_hx_printf("e1000: %s: ERROR: " fmt, (NIC)->name ,__VA_ARGS__)

#ifdef E1000_DEBUG
#define E1000_DBG(NIC, fmt, ...) \
	_hx_printf("e1000: %s: DEBUG: " fmt, (NIC)->name ,__VA_ARGS__)
#define DEBUGOUT(fmt, ...)	_hx_printf(fmt ,__VA_ARGS__)
#define DEBUGFUNC()		_hx_printf("%s\r\n", __func__);
#else
#define E1000_DBG(HW, ...)	do { } while (0)
#define DEBUGFUNC()		do { } while (0)
#define DEBUGOUT(fmt, ...)	do { } while (0)
#endif

#define e1000_readl(reg) readl(reg)
#define e1000_writel(val, reg) writel(reg,val)

/* I/O wrapper functions */
#define E1000_WRITE_REG(a, reg, value) \
	e1000_writel((value), ((a)->hw_addr + E1000_##reg))
#define E1000_READ_REG(a, reg) \
	e1000_readl((a)->hw_addr + E1000_##reg)
#define E1000_WRITE_REG_ARRAY(a, reg, offset, value) \
	e1000_writel((value), ((a)->hw_addr + E1000_##reg + ((offset) << 2)))
#define E1000_READ_REG_ARRAY(a, reg, offset) \
	e1000_readl((a)->hw_addr + E1000_##reg + ((offset) << 2))
...
struct e1000_hw {
	...
	struct e1000_hw* pNext; /* Next ptr in globl list. */
	//struct eth_device *nic;
	__ETHERNET_INTERFACE* nic;
#ifdef CONFIG_E1000_SPI
	...
	/* 
	 * Physical resource of this NIC,used by HelloX
	 * device and network framework.
	 * May duplicate with other variables in this structure.
	 */
	__u16 ioport_s;    //IO port start address.
	__u16 ioport_e;    //IO port end address.
	int int_vector;    //Interrupt vector.
	LPVOID memaddr_s;  //Start memory mapping address.
	LPVOID memaddr_e;  //End memory mapping address.

	/*
	 * TX/RX descriptors and packet buffer.
	 */
...
/*
 * Global routines that used by other modules.
 */
