//***********************************************************************/
//    Author                    : Garry
//    Original Date             : Jan 31, 2021
//    Module Name               : ata.h
//    Module Funciton           : 
//                                General ATA constants, structures, and
//                                definitions, common routines, are defined
//                                in this file.
//
...
typedef struct _IDENTIFY_DEVICE_DATA {
	...
	USHORT Signature : 8;
	USHORT CheckSum : 8;
} __IDENTIFY_DEVICE_DATA;
#pragma pack(pop)

/* General harddisk object. */
typedef struct tag__ATA_DISK_OBJECT {
#if defined(__CFG_SYS_SMP)
	__SPIN_LOCK spinlock;
#endif
	/* Identify device data. */
	__IDENTIFY_DEVICE_DATA IdentifyData;
	/* Sector's size in byte. */
	unsigned long sector_size;
	/* disk's volume in sectors number. */
	unsigned long total_sectors;
	/* Current sector index. */
	unsigned long curr_sector;
	/* How many times the disk is opened. */
	unsigned long open_counter;
	/* The controller that controls this disk. */
	void* controller;
}__ATA_DISK_OBJECT;

/* ATA disk's device name prefix. */
#define ATA_DISK_NAME "\\\\.\\ATADISK%d"
...
typedef struct tag__PARTITION_EXTENSION {
	...
	/* Partition type. */
	BYTE PartitionType;
	/* Start sector in disk. */
	uint32_t dwStartSector;
	/* How many sectors. */
	uint32_t dwSectorNum;
	/* Current pointer. */
	uint32_t dwCurrPos;
	/* Hard disk's index nuber. */
	int nDiskNum;
	/* Hard disk this partition resides on. */
	__ATA_DISK_OBJECT* pHardDisk;
	/* How many times opened. */
	__atomic_t open_counter;
#if defined(__CFG_SYS_SMP)
...
/* Default sector size. */
#define STORAGE_DEFAULT_SECTOR_SIZE 512

/* Maximal read/write size of an ATA disk. */
#define STORAGE_MAX_RW_SIZE (64 * 1024)
