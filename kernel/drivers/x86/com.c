...
/*
 * COM device array,each COM/USART/UART
 * interface existing in systen corresponding
 * to one entry of this array.
 */
#define COM_DEVICE_NUM 1
 static __COM_CONTROL_BLOCK ComCtrlBlock[] = {
		  ADD_COM_DEVICE(COM1_NAME,COM1_BASE,COM1_INT_VECTOR),
		  //ADD_COM_DEVICE(COM2_NAME,COM2_BASE,COM2_INT_VECTOR),
		  /* More entry could be append. */
 };
 ...
 static BOOL InitializeCOM(WORD base)
 {
	 ...
	 __outb(0x0, base + 1);
	 /* Reset DLAB bit,and set data bit to 8,one stop bit,without parity check. */
	 __outb(0x03, base + 3);
	 /* Enable FIFO, clear them, with 14 bytes as threshold. */
	 __outb(0xC7, base + 2);
	 /* Enable DTR,RTS and Interrupt. */
	 __outb(0x0B, base + 4);
 ...
 static VOID DAIntHandler(__COM_CONTROL_BLOCK* pCtrlBlock)
 {
	 ...
	 /* Drain out all data from COM interface,since the buffer is full. */
	 while (COMRecvByte(pCtrlBlock->wBasePort + 5) & 1);
 ...
 static BOOL COMIntHandler(LPVOID lpESP, LPVOID lpParam)
 {
	 ...
	 UCHAR isr = 0, lsr = 0, lcr = 0, mcr = 0, msr = 0, scr = 0, ier = 0;

	 isr = COMRecvByte(pCtrlBlock->wBasePort + 2);
	 lsr = COMRecvByte(pCtrlBlock->wBasePort + 5);
	 lcr = COMRecvByte(pCtrlBlock->wBasePort + 3);
	 mcr = COMRecvByte(pCtrlBlock->wBasePort + 4);
	 msr = COMRecvByte(pCtrlBlock->wBasePort + 6);
	 scr = COMRecvByte(pCtrlBlock->wBasePort + 7);
	 ier = COMRecvByte(pCtrlBlock->wBasePort + 1);
	 //_hx_printk("COM int:isr = %X, lsr = %X, lcr = %X, mcr = %X, msr = %X, scr = %X, ier = %X.\r\n",
	 //	 isr, lsr, lcr, mcr, msr, scr, ier);
	 if (isr & 1)
	 {
		 /* No interrupt raise. */
		 return FALSE;
