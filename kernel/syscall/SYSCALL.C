...
/*
 * System call implemented in other kernel module
 * use this routine to register them into kernel.
 */
...
/*
 * If can not find a proper service routine in 
 * master kernel,then try to dispatch the system 
 * call to other module(s) by searching system call 
 * range array.
 */
static BOOL DispatchToModule(LPVOID lpEsp,LPVOID lpParam)
{
	...
	/* Can not find a system call range to handle it. */
	return FALSE;
}
...
BOOL SyscallHandler(LPVOID lpEsp,LPVOID lpParam)
{
	...
	/* 
	 * Enable local interrupt,since it disabled 
	 * by CPU automatically when syscall instruction(int
	 * under x86 for an example) invoked.
	 */
	...
	{
		_hx_printf("SyscallHandler:unknown syscall[id = 0x%0X].\r\n",
			pspb->syscall_num);
		goto __TERMINAL;
	}
	bResult = TRUE;
__TERMINAL:
	/* Disable local interrupt as the syscall entered. */
