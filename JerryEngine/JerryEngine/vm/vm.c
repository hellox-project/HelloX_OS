        ...
        ecma_length_t arg_list_len) /**< length of arguments list */
{
  ...
  //ecma_value_t stack[call_stack_size]; /* Replace dynamic array under MSVC. */
  __CREATE_DYNAMIC_ARRAY(ecma_value_t, stack, call_stack_size);
  frame_ctx.registers_p = stack;

  ecma_value_t retval = vm_execute (&frame_ctx, arg_list_p, arg_list_len);
  __RELEASE_DYNAMIC_ARRAY(stack);
  return retval;
} /* vm_run */
