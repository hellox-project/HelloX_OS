...
VOID  MakeShortEntryName(CHAR* pShortName,CHAR* pLongName,INT nNum)
{
	...
	memcpy(pShortName,pLongName,6);

	sprintf(szNum,"~%d",nNum);
	strcat(pShortName,szNum);

	pDotPos = strstr(pLongName,".");
	if(pDotPos)
	{
		pDotPos ++;
		strcat(pShortName,pDotPos);	
	}
	else
	{
		strcat(pShortName,"   ");	
...
BOOL GetSubDirectory(CHAR* pFullName,DWORD dwLevel,CHAR* pSubDir)
{
	...
	BYTE       buffer[MAX_FILE_NAME_LEN]   = {0};  //Contain sub-directory name temporary.
	DWORD      i			               = 0;
	DWORD      j                           = 0;
...
BOOL ConvertName(__FAT32_SHORTENTRY* pfse,CHAR* pResult)
{
	int i,j = 0;
	...
	if((pfse->FileName[8] == ' ') && 	(pfse->FileName[9] == ' ') &&	(pfse->FileName[10] == ' '))  //If no extension.
	{		
		goto _END;
	}

	if(j <= 7) //Append the dot of name string.
	{
		pResult[j ++] = '.'; 
	}
...
VOID CombinLongFileName(__FAT32_LONGENTRY* plongEntry,CHAR* pFileName)
{	
	CHAR  szBuf[32] = {0};
	BYTE  nValue    = 0;
	INT   i         = 0;
...
BOOL DirEntryIsExist(__FAT32_FS* pFat32Fs,DWORD dwStartCluster,CHAR* pEntryName,BOOL bLong)
{
	...
	while(!IS_EOC(dwCurrClus))  //Main loop to check the root directory.
	{
		...
		for(i = 0;i < pFat32Fs->SectorPerClus * 16;i ++)
		{
			...
			{
				//long file name
				for(INT i = nLongEntryNum-1;i >= 0 ;i--)
				{
					CHAR szTemp[128]  = {0};

					CombinLongFileName(szLongEntry[i],szTemp);
			...
			{				
				ConvertName(pfse,szFileName);
			}
										
			if(strcmpi(pEntryName,szFileName) == 0) 
			{
				bResult = TRUE;
				goto __TERMINAL;
			}
	...
	while(!IS_EOC(dwCurrClus))  //Main loop to check the root directory.
	{
		...
		for(i = 0;i < pFat32Fs->SectorPerClus * 16;i ++)
		{
			...
			else
			{
				...
				if(strcmpi(pFileName,szFileName) == 0) 
				{
					bFind = TRUE;				
				}									
			}
...
VOID  InitLongEntry(FAT32_LONGENTRY* pLongEntry,INT  nLongEntryNum,CHAR* pszName,BYTE bSumofSfn)
{
	...
	{
		FAT32_LONGENTRY*  pTempEntry                               = &pLongEntry[nLongEntryNum-i-1];
...
HANDLE APIENTRY OpenVhdFile(LPCTSTR pVhdFile,DWORD dwPartitionSatrt)
{
	...
	pFat32Fs->dwPartitionSatrt = dwPartitionSatrt;
...
BOOL CreateFatDir(__FAT32_FS* pFat32Fs,DWORD dwStartCluster,CHAR* pszDirName,BYTE Attributes)
{
	...
	bLongDir = (strlen(pszDirName) > FAT32_SHORTDIR_FILENAME_LEN);	
	if(bLongDir == FALSE)
	{
	...
	pEntryArry = (__FAT32_LONGENTRY*)LocalAlloc(LPTR,nEntryNum*sizeof(__FAT32_LONGENTRY));
	if(nEntryNum > 1)
	{
		BYTE bSfnSum = 0;
		INT  i       = 1;
		...
		{			
			...
			ZeroMemory(szShorDir,sizeof(szShorDir));
		}
...
BOOL APIENTRY AddDirToVhd(HANDLE hVhdObj,LPCSTR pDirName)    //Create directory.
{
	...
	return CreateFatDir(pFat32Fs,dwDirCluster,SubDirName,FILE_ATTR_DIRECTORY);
}
...
BOOL APIENTRY AddFileToVhd(HANDLE hVhdObj,LPCSTR pSrcFile,LPCSTR pDstFile)
{
	...
	//return CreateFatDir(pFat32Fs,dwDirCluster,SubDirName,FILE_ATTR_DIRECTORY);
	if(!CreateFatDir(pFat32Fs,	dwDirCluster,FileName,FILE_ATTR_ARCHIVE))
	{
		return FALSE;		
	}
