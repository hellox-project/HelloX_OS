...
BOOL  APIENTRY WriteBootSec(HANDLE hDiskDrive,LPCTSTR pAppPath,UINT  nReserved,UINT nTotalSector)
{
	...
	wsprintf(szBootFile,L"%sbootsect_hdd",pAppPath);
...
VOID  APIENTRY SetProcessInfo(HWND hWnd,UINT nPID)
{
	switch(nPID)
	{
		...
		case PROCESS_IMPORT_VHD:
			{
			wsprintf(g_szProcessInfo,L"正在生成BOOT信息，正在倒入初始化文件.....");
			}
			break;
		default:
			return;
...
HANDLE APIENTRY OpenVhdFile(LPCTSTR pVhdFile);
VOID   APIENTRY CloseVhdFile(HANDLE hObj);
BOOL APIENTRY AddDirToVhd(HANDLE hVhdObj,CHAR*   pszDirName);
BOOL APIENTRY AddFileToVhd(HANDLE hVhdObj,LPCSTR pDiskFile);



int APIENTRY testFat32()
{
	//ImportFilesToVhd(L"D:\\work\\HelloX\\BootTest\\vdisk.vhd",L"D:\\work\\HelloX\\BootTest\\import\\");

	//HANDLE hVhdObj = OpenVhdFile(L"D:\\work\\HelloX\\BootTest\\vdisk.vhd");

	//AddDirToVhd(hVhdObj,"C:\\DIR2");
	//AddFileToVhd(hVhdObj,"C:\\DIR2\\TEST.DAT");
	
	//AddFileToVhd(hVhdObj,"C:\\dir2\\test4.dat");
...
{
	USHORT  szBuf[60] = {0};
	CHAR   szChar[]   = {"AMP3FO~1TXT"};//"amp3foryatoumadebyfgd20090808summer.txt"
	INT    nStrlen    = strlen(szChar);	
	...
	byte sum = sum_sfn((LPBYTE)szChar);
	//int n = checksum((USHORT*)szChar,nStrlen/2);
	//return testFat32();
